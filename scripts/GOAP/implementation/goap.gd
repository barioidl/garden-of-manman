...
var all_mutations := {}
var mutations_path := "res://data/goap/goap mutations.json"
#contains score, weights and name for reference
func save_mutations(data:Dictionary):
	...
	if current_data.score >= data.score:
		_print('rejected disadvantage mutation')
		return
	all_mutations[data_name] = data
